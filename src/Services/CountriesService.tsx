...
const getCountries = () =>
  regions.concat(
    countries
      .filter(m => m.alpha2Code !== "US")
      .map(m => ({ label: m.name, value: m.alpha3Code }))
  );

// const url =
//   //"https://raw.githubusercontent.com/tarraq/JSON-data-arrays/master/countries/english/countries-key-value.json";
...
//   }
//   let items: ICountryInfo[] = await response.json();
//   let formatted = items.map(m => ({ label: m.name, value: m.alpha3Code }));
//   return regions.concat(formatted);
// };
